...
constexpr int32_t RUNTIME_VERSION = 1;
...
bool DryerNvsStore::loadCoolingSettings(DryerNvsCoolingSettings *v) const
{
    ...
    size_t size=sizeof(loaded);err=nvs_get_blob(h,"settings",&loaded,&size);
    nvs_close(h);
    if (err != ESP_OK || size != sizeof(loaded) || loaded.dry_temp_c<DRYER_CFG_TEMP_MIN_C || loaded.dry_temp_c>DRYER_CFG_TEMP_MAX_C ||
        ...
        loaded.http_server_port<1 || loaded.http_server_port>65535) return false;
    *v = loaded;
    return true;
}
...
bool DryerNvsStore::loadRuntime(DryerNvsRuntime *v) const
{
    ...
    nvs_close(h);
    if (err != ESP_OK || version != RUNTIME_VERSION ||
        loaded.remain_min < 0 || loaded.remain_min > 1440 ||
        loaded.set_temp_deci_c < 100 || loaded.set_temp_deci_c > 900 ||
        loaded.preheat_temp_c < 30 || loaded.preheat_temp_c > 90 ||
